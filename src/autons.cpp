...
void defensive_side_mess_up()
{
    ...
    chassis.moveToPoint(chassis.getPose().x, level, 1200, {.maxSpeed = DRIVE_SPEED});
...
void doing_6_ball()
{
    ...
    wings.set_value(false);
    chassis.turnToHeading(190, 300, true, 60);
    chassis.waitUntilDone();
    chassis.tank(60, 60);
    pros::delay(300);
    ...
    wings.set_value(false);
    chassis.moveToPoint(48, -48, timeout);
    chassis.moveToPoint(24, -24, 1200, {.maxSpeed = 110});
    chassis.turnToPoint(48, -12, 1000);
    chassis.waitUntilDone();
    ...
    chassis.moveToPoint(2, -24, 800);
    chassis.waitUntilDone();
    chassis.setPose(6, -18, -90);
    pros::delay(300);

    // shoot second
    chassis.moveToPoint(24, -24, 900, {.forwards = false});
...
void skills2()
{
    ...
    for (int i = 0; i < left_pushes; i++)
    {
        wings.set_value(true);
        pros::delay(WING_ACTIVATE_DELAY);
        chassis.moveToPose(-48 + 8.25 - 10 - 4 * i, -3, 90, 1500, {.forwards = false, .minSpeed = 127});
        chassis.waitUntilDone();
        wings.set_value(false);
        pros::delay(WING_ACTIVATE_DELAY);
    ...
    for (int i = 0; i < right_pushes; i++)
    {
        wings.set_value(true);
        pros::delay(WING_ACTIVATE_DELAY);
        chassis.moveToPose(-48 + 8.25 - 11 - 3 * i, 10 + 3, 90 + 10, 1500, {.forwards = false, .minSpeed = 127});
        chassis.waitUntilDone();
        wings.set_value(false);
        pros::delay(WING_ACTIVATE_DELAY);
...
/// @param angular_movement how many degrees you wnat to turn clockwise
void angular_pid_tuning(int angular_movement)
{
    const int angular_accuracy = 100;
    chassis.setPose(0, 0, 0);
